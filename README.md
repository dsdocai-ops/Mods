...
- **Shader support**: shaderpacks need a shader loader (Iris on Fabric, Oculus on Forge) - these are third-party, independently-owned mods (Iris/Sodium/Oculus are LGPL-3.0) that Omega does **not** bundle or install silently. The **Shaders** tab offers a clearly-attributed, opt-in **Install shader loader** button that fetches them from Modrinth into that instance when *you* choose to. Import `.zip` shader packs there too; pick the active one in-game under Video Settings → Shader Packs (that menu lives inside the game, the launcher can't drive it).
...
The status card's icon is [`build/discord-rich-presence-icon.png`](build/discord-rich-presence-icon.png) - a rounded-square gradient mark with a bold Ω, not the app's black taskbar/dock icon (`build/icon.png`), which reads as a near-invisible black square on Discord's own dark theme. Uploaded under the application's **Rich Presence → Art Assets** as `omega_icon`, matching `LARGE_IMAGE_KEY` in `main/discordPresence.ts`.
...
- **The Omega mod is built in, Lunar-style.** The launcher ships with its own companion mod jars (CI stages them into the packaged app via electron-builder `extraResources`; dev clones fall back to fetching them from the repo's rolling release) and (re)installs the right one into every Fabric/Forge instance on creation and before every launch — its features are launcher features, edited first-class in the **Features** tab (which reads/writes the same `config/omega-client.json` the in-game Right Shift menu uses). Disabling it in the Mods tab is respected: the refresh updates the `.disabled` file instead of re-enabling it. The one jar we don't own that's fetched automatically is Fabric API (Fabric builds of the Omega mod need it to run at all) — a required dependency of our own mod, from Modrinth, cached per MC version, only when missing. Shader loaders (Iris/Sodium on Fabric, Oculus on Forge) are third-party mods we deliberately do **not** auto-install: the **Shaders** tab offers an opt-in, attributed **Install shader loader** button so pulling in other people's mods is always the user's explicit choice. Beyond those, other mods only arrive when you import your own jars or explicitly install one from the Mods tab's **Discover** view (which fetches from the Modrinth API).
...
**Paid cosmetics.** Settings' Cosmetics section has a "Buy a cosmetic" button (a hardcoded link in `shared/cosmetics.ts`, same restricted-to-`https://` `external:open` path as sponsor placements) and a license-key redeem field - paste in the key you're given after buying, and it unlocks a cosmetic other Omega Client players see on you in-game: a recolored nametag badge, pixel-art gear (hat, cape, wings) rendered on your player model like an extruded Minecraft item texture, or a cape wrapping a real UV-mapped PNG texture (same presence mechanism as the free Ω badge - needs a server/proxy relaying the presence channel either way). On redemption, `unlockCosmetic()` (`main/licensing.ts`) persists the cosmetic to a local `licenses.json` and stamps `ownedCosmeticId` into every instance's `config/omega-client.json`, the same file the in-game Right Shift menu reads/writes.

**Known, accepted limitation**: like every other toggle in this app, this isn't backed by server-side enforcement - a hand-edited config can grant a cosmetic without paying, exactly as it could already flip `fullbrightEnabled`. Proportionate to a vanity-only feature, consistent with this project generally not attempting anti-tamper enforcement. `mod/common/.../presence/CosmeticCatalog.java` currently ships placeholder cosmetics covering every kind and both gear rendering styles (`gold_badge`/`azure_badge` badges, `crimson_cape`, `seraph_wings`, `obsidian_top_hat` procedural pixel-art gear, `azure_charm_hat` a true-3D voxel-art hat, `starlit_cape` a real UV-mapped PNG texture) so the pipeline is testable end to end; real cosmetic art/copy is a content decision, and adding one of an existing kind is a data-only change (see the `generate-cosmetic` skill).

**Coins.** A second, in-app way to unlock a cosmetic, alongside the direct Stripe-link-per-cosmetic flow above. The Cosmetics page shows a coin balance (the pixel-art coin icon + count, top-right of the page) and every cosmetic in the catalog has a `coinPrice` (`shared/cosmetics.ts`); clicking an unowned one spends coins and unlocks+equips it immediately, no license key involved. Coins themselves are added only by redeeming a signed top-up code (format `coins:<amount>-<nonce>-<hmac>`, same shape as a cosmetic license key) generated by the private `scripts/generate-coin-code.cjs` after you've confirmed a real Stripe payment for a coin pack - there is no "just set a number" path anywhere in the app. The balance lives in `wallet.json` (`main/wallet.ts`, userData dir) signed with an HMAC (`WALLET_SECRET`, keep in sync with the generator script) covering both the coin total and the list of already-redeemed codes together: hand-editing the coin number, or deleting a redeemed code to replay it, breaks the signature and the whole wallet resets to zero on the next read rather than trusting the tampered value - a mistake wipes your balance instead of growing it. This is a meaningfully stronger gate than the plain self-reported `ownedCosmeticId` config above (casual file-editing is defeated, not just discouraged), but it's still the same fundamental trade-off as `LICENSE_SECRET`: the verification secret ships inside the client, so it's not real DRM against someone willing to read the source and recompute the HMAC - proportionate to a vanity-only economy, not something worth standing up a backend for.

Clicking the balance pill (or the "Buy coins" button next to the redeem field) opens a coin shop (`CoinShopModal.tsx`) listing four packs from `shared/coinPacks.ts` - 1,000/\$10, 2,800/\$25, 6,500/\$50 ("Popular"), 14,500/\$100 ("Best value"), coins-per-dollar rising with pack size like most game currency shops - each drawn as a pile of the same coin icon (more coins, taller pile, no new art needed). **No backend exists yet to verify a Stripe payment and auto-credit coins**, so each pack's price button just opens that pack's own Stripe Payment Link (a separate product per pack, unlike cosmetics' single link, since these differ by price) and the buyer still redeems the code they're handed afterwards through the same "Get coins" field - the modal only replaces picking a link, not the manual redeem step. Once a real backend exists to confirm the Stripe payment webhook, it should mint the coin code itself (same `WALLET_SECRET` formula) and skip the "wait for a human to hand you a code" step entirely.
