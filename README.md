...
- **Versions**: the launcher installs every mainline Minecraft release straight from Mojang's live manifest, and the Omega mod's per-version support is declared in a manifest (`omega-versions.json`) instead of hardcoded. Versions are tiered — **main** (flagship, full features, today 1.20.1), **bridge** (thin ports with core features), or unlisted (vanilla-playable, Omega skipped). Shipping a new bridge port is just one porting step + one manifest entry; every installed launcher picks it up on next startup with zero app update. See [`docs/VERSIONS.md`](docs/VERSIONS.md) for the full design.
