...
**Paid cosmetics.** Settings' Cosmetics section lets you redeem a Stripe Checkout Session id for a cosmetic badge other Omega Client players see next to your name in-game (same nametag mechanism as the free Ω presence badge - needs a server/proxy relaying the presence channel either way). The flow:

- `main/licensing.ts`'s `redeemLicenseKey()` POSTs the redeemed key to a URL *you* configure and deploy yourself (Settings -> Cosmetics -> "Stripe verify endpoint URL") - see [`server/stripe-verify/README.md`](server/stripe-verify/README.md) for the full walkthrough (create a Stripe Product/Price, a Payment Link, deploy the included reference verify function to Vercel, wire up the price-to-cosmetic map). A Stripe *secret* key can issue refunds and read customer data, so it can never live in a distributed desktop client - this is why a small server is unavoidable here, and why that URL is empty (feature inert) until you deploy your own.
- On a confirmed payment, `unlockCosmetic()` persists the cosmetic to a local `licenses.json` and stamps `ownedCosmeticId` into every instance's `config/omega-client.json`, the same file the in-game Right Shift menu reads/writes - no separate cosmetic-selection UI needed, it just flows through the same config the mod already reads at launch.
- **Known, accepted limitation**: cosmetic ownership is self-reported from that same JSON, the identical trust model every other toggle in this app already uses (a hand-edited config can grant one for free, exactly as it could already flip `fullbrightEnabled`). Proportionate to a vanity-only feature - not being hardened, consistent with this project generally not attempting anti-tamper enforcement.
- `mod/common/.../presence/CosmeticCatalog.java` currently ships two placeholder badge colors (`gold_badge`, `azure_badge`) so the pipeline is testable end to end; real cosmetic art/ids are a content decision, not a code change - see `server/stripe-verify/cosmetic-price-map.json` for where a new one gets wired to an actual Stripe Price.

An optional cloud-sync subscription was also scoped as a viable next step but is a meaningfully bigger undertaking (a whole hosted backend, not just a stateless verify function) - not started.
