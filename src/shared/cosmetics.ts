 ...
 * (main/licensing.ts) - coins (below) are an additional, separate way to unlock a cosmetic.
 */
export const STRIPE_COSMETIC_PAYMENT_LINK_URL = "https://buy.stripe.com/REPLACE_ME";

/**
 * Where to buy a coin pack - same restricted-to-`https://` `external:open` path as above. Replace
 * with your real Stripe Payment Link for coins. After payment, hand the buyer a code generated by
 * scripts/generate-coin-code.cjs; they redeem it on the Cosmetics page to add coins to their balance
 * (main/wallet.ts), then spend coins there to unlock any cosmetic in the catalog above.
 */
export const STRIPE_COINS_PAYMENT_LINK_URL = "https://buy.stripe.com/REPLACE_ME_COINS";
