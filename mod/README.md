...
- **`network/PresenceNetworking.java`** (presence badge) - written against Forge 47.2's ChannelBuilder/EventNetworkChannel refactor plus a raw ServerboundCustomPayloadPacket send; a fresh never-compiled guess of the same kind the particle Mixin wiring was. `EntityRendererMixin`'s `renderNameTag` target is the same class of guess.
