...
This is groundwork, not the finish line: `common/` still only holds the zero-Minecraft-import classes listed above, and no feature code has moved into it yet. What this toolchain switch unlocks for later is putting Minecraft-touching code in `common/` compiled once against official mappings, with Loom remapping it into each loader's final jar - the actual point of adopting Architectury. The exact `architectury_plugin_version`/`architectury_loom_version` pins in `gradle.properties` are the long-standing stable channel Architectury's docs/templates use for Loom 1.6.x / Minecraft 1.20.1, but - like the rest of the Forge module's history - this project has no way to resolve Gradle plugins outside CI, so treat the first CI run after this change the same way the original Forge port's first two rounds were treated: a real compile is the actual verification, not a confidence claim made here.
