...
| **Omega presence badge** | An Ω (brand red) rendered left of the nametag of players known to be on Omega Client. Backend-less design ceiling, stated plainly: a client cannot know what another client runs on a plain vanilla server - presence is announced over a custom `omega-client:presence` payload channel, which is silently dormant unless the server/proxy relays it; a future hosted presence API can feed the same set (`common/presence/OmegaPresence`). Paid cosmetics ride the same channel: a badge kind recolors the Ω, gear kinds (hat/cape/wings) are pixel art (`common/presence/CosmeticPixelArt`) extruded onto the wearer's model like a Minecraft item texture - each opaque pixel a colored cell, transparency cutting the silhouette (`common/presence/CosmeticGeometry`) - via Fabric API's feature-renderer callback / Forge's `AddLayers` event, no extra Mixins (`fabric/render/CosmeticFeatureRenderer`, `forge/render/CosmeticRenderLayer`). Cape and wing geometry sways/flaps every frame - a stylized procedural rotation about each cosmetic's shoulder/collar pivot, scaled by distance from that pivot and by the wearer's vanilla limb-swing-amount (`common/presence/CosmeticAnimation`), not a physics or cloth simulation; hats stay rigid by design. Capes/wings can also opt into a sparse colored particle trail from their tip (`common/presence/CosmeticTrail` for the pure world-placement math, `DustParticleEffect`/`DustParticleOptions` in the two renderers for the actual spawn) - an authoring choice per cosmetic, not automatic. Toggleable (default on, one switch covers badge + gear). |
