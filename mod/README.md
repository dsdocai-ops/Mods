...
| **Info HUD** | Coordinates, FPS, ping, facing direction, a CPS (clicks-per-second) counter, and a WASD+space+click keystroke display - each line individually toggleable. All of this is already visible via vanilla's F3 debug screen or tab list - this is just an always-on, friendlier subset of it. The CPS counter *reads* your click rate (sampled per-frame, since fast clicks are shorter than a game tick); it never generates clicks. |
