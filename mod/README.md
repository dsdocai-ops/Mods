...
| **Cosmetics** | A "Cosmetics..." menu screen where you pick which badge color (None/Gold/Azure - see `common/presence/CosmeticCatalog`) shows in place of the default red Ω for other Omega Client players. Click the one button to cycle through the options, same pattern as Visual Settings' highlight color. The launcher's own Settings → Cosmetics section (buy + redeem a license key) sets the same value; this screen just makes it switchable in-game too. |
