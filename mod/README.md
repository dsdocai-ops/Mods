...
If something doesn't compile against the pinned `yarn_mappings` in `gradle.properties`, the most likely culprit is `GameOptions.getGamma()` / `getFov()` in `FullbrightFeature.java` / `FovZoomFeature.java` - Yarn's exact accessor names for these two fields are the one place in this code I have moderate rather than high confidence in. Your IDE's autocomplete on `client.options.` will show the real names for your exact `yarn_mappings` build if they've drifted.
