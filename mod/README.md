...
| **Omega presence badge** | An Ω (brand red) rendered left of the nametag of players known to be on Omega Client. Backend-less design ceiling, stated plainly: a client cannot know what another client runs on a plain vanilla server - presence is announced over a custom `omega-client:presence` payload channel, which is silently dormant unless the server/proxy relays it; a future hosted presence API can feed the same set (`common/presence/OmegaPresence`). Toggleable (default on). |
| **Cosmetics visibility** | A "Cosmetics..." menu with a master on/off switch, a "My cosmetics" / "Other players' cosmetics" split, and a per-slot toggle (Hats/Capes/Wings) - all default on. Purely controls what `CosmeticRenderer` draws for whoever's already broadcasting a worn cosmetic; separate from the Ω badge toggle above and from equipping a cosmetic (done in the launcher's Cosmetics page). |
