...
Each loader module still has its own full implementation of every feature (ModConfig, FullbrightFeature, the schematic tool, etc.) - **not yet** a single shared implementation, though both modules now build through **Architectury Loom** rather than plain Fabric Loom / ForgeGradle (see "Architectury toolchain" below). Here's why the feature code itself is still duplicated: Fabric mods are conventionally written against Yarn mappings (community-maintained names for Minecraft's classes), Forge mods against Mojang's own official mappings - different names for the same underlying game classes. Naively sharing source between a Yarn-mapped module and an officially-mapped one isn't safe without a remapping layer, which is what Architectury Loom provides - but actually moving code into `common/` to use it is follow-up work, not yet done. Until then, the lower-risk choice remains two independent, loader-idiomatic implementations, sharing only the classes with zero Minecraft API surface at all (`SchematicData`, `SchematicBlockEntry`, `SessionInfo`, `ParticleCategory` - plain data/logic holders, safe to compile once and use from both).
...
Actual feature code has started moving into `common/` (`FullbrightFeature`, `FovZoomFeature`, `ToggleSprintFeature`, `InfoHudFeature` so far, out of roughly two dozen duplicated files total) - each one decoupled from the loader-specific `ModConfig` type (raw parameters, or a small record like `HudSettings` for classes reading many flags) so the migration doesn't also require unifying `ModConfig` itself. The load-bearing rule discovered while doing this: **a method in `common/` can take or return primitives, records-of-primitives, or types already living in `common/` - never a raw Minecraft-mapped type (`GuiGraphics`, `Minecraft`, `KeyMapping`, etc.) in its public signature.** `fabric/`'s compile-time view of `common/`'s classes is the *unremapped* official-mapped signature (the `common` dependency configuration, `namedElements`, is common's own native compiled output - only the final shadowJar-merged runtime artifact goes through `transformProductionFabric`'s remapping), so a parameter typed `GuiGraphics` there is a different, unrelated type from Fabric's own `DrawContext`-typed callback parameters, even though they're "the same" class once fully remapped at the bytecode level. `InfoHudFeature` hit this directly: its `tick()` and `render()` methods originally took `Minecraft`/`GuiGraphics` parameters and failed Fabric's compile with "cannot access GuiGraphics ... class file ... not found". The fix was to compute and cache only plain data in `common/` (calling `Minecraft.getInstance()` *internally*, which is fine - it's only the public signature that can't carry the type across) and keep the handful of lines that actually call `GuiGraphics`/`DrawContext` drawing methods as small per-loader glue in `OmegaClient.java`/`OmegaClientForge.java`, reading the cached state through plain getters. Anything with a similar shape (screens, other renderers) will need the same split - the render/GUI plumbing stays loader-specific by necessity, but the logic behind it doesn't have to.
