...
/**
 * Forge-side twin of the Fabric FovZoomFeature - `options.fov()` accessor per CI's correction of
 * the original direct-field guess (same as FullbrightFeature), same re-sync-from-live-value fix
 * (see the Fabric class's javadoc for why capturing the base FOV only once would go stale the
 * moment the player touches vanilla's own FOV slider).
 */
